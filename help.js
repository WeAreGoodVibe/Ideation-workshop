...
window.HELP = (function () {
  ...
  var tips = {
    view: {
      ...
      prompts: 'What to paste into Claude to build each idea: an interview prompt for whoever owns the work, the skill or scheduled task itself, and the first message to send.',
    ...
    action: {
      ...
      vote: 'Add or remove a vote. One dot is one vote. The number of votes per person is set in the Voting bar at the top of this view, or in Settings, and can be any number.',
      ...
      resetvotes: 'Clears every vote on the board so the room can vote again, on a shorter list or with more votes each. The ideas stay. There is no undo.',
      savedots: 'Sets how many votes each person gets. Save and every phone updates at once, with the votes already cast left alone.',
      genprompts: 'Claude reads the ideas on the board and writes, for each one, a prompt that interviews the owner, the skill or scheduled task itself, and the first message to send. A minute or two.',
      genpromptsall: 'Throws away every pack on this board and writes them all again from scratch.',
      genprompt: 'Write this one idea again from scratch.',
      copypack: 'Copy this text so you can paste it straight into Claude.',
      ...
      copylink: 'Copy the link participants open on their phone, for anyone whose camera will not scan the square.',
      bigqr: 'Fill the screen with the QR code and the join code so the back of the room can scan it.',
      sendlink: 'We email you a sign-in link. No password. Facilitators only; participants scan the square instead.',
      joincode: 'Join this workshop with your name. Scanned from the screen, the code is already filled in.',
      ...
      openws: 'Open this workshop on the board. It comes back exactly as it was left: every idea, vote, system, phase and the transcript. Nothing expires, so you can pause a session and pick it up weeks later.',
      dupws: 'Make a new workshop from this one: its settings, systems, process phases and prepared second viewpoint ideas come across. The previous session\'s opportunities, votes, transcript and prompt packs do not. Use it to keep a prepared shell per client and start each session fresh.',
      delws: 'Permanently delete this workshop and everything in it: ideas, votes, systems, phases, the second viewpoint and the transcript. There is no undo and no backup. Export to Excel first if there is anything worth keeping.',
  ...
  var howto = {
    ...
    opportunities: ['Every idea lands here with a number. The badge in the sidebar bumps when one arrives.', 'Click a title to rename it. Press Edit for the full form. Table mode is a spreadsheet you can type into.', 'In block 6 use the plus and minus for dot votes. Press Validate when the room confirms an idea.', 'Use the filters to show only Finance, only Purchasing, or only one status.', 'Votes each and Reset votes sit at the top of this view. Reset clears every vote so the room can go again on a shorter list; raising Votes each gives everyone more without anyone refreshing.', 'Press Export Excel at the end of the session.'],
    prompts: ['Press Generate once the room has voted and you know which ideas matter. Claude writes a pack for each one.', 'Every pack has three parts. The interview prompt is what the owner of that work pastes into Claude so Claude interviews them. The artefact is the skill or scheduled task itself. The first message is what they send once it exists.', 'Press Copy on any part and paste it straight into Claude.', 'Press Rewrite on a card to write that one again. Redo all throws away every pack and starts over.', 'Export Excel afterwards: the packs come out as their own tab.'],
  ...
  var guide = [
    ...
    { title: 'On the day', steps: ['Open Live capture and start your transcript source: Wispr Flow meeting, or dictation into the box.', 'Plug in the projector and press P for presentation mode.', 'Open Run sheet and press Start the session.', 'Follow the blocks. Press Next block when each one is done. The page moves to the right view for you.', 'Block 7: open Second viewpoint and press Reveal.', 'Block 8: open Prompts and skills and press Generate, then Opportunities and press Export Excel. Send the file to the room.'] },
    { title: 'What the words mean', terms: [['Opportunity', 'One idea: a task Claude could carry. It has a title, a team, a phase, a surface, a build type and a status.'], ['Function', 'Which team it belongs to: one of the teams named when the workshop was created, Both, or Org-wide.'], ['Phase', 'Where in the process the idea sits, for example Accounts payable or Purchase orders. The Process walk view counts ideas per phase.'], ['Surface', 'Which part of Claude does the work: Chat, a Project, a Scheduled Task, Cowork, a Skill, or a Connector setup.'], ['Build type', 'What has to be made: a Skill, a Scheduled task, a Setup, a Project, or a Workflow redesign.'], ['Status', 'Open, Validated, Emerging, Parked or Merged. Hover any status tag for the meaning.'], ['Second viewpoint', 'Ideas from outside the room: what Claude writes from today’s transcript, plus any prepared in advance for this workshop. Each one says why the room did not raise it.'], ['Votes', 'Dot votes. Each person gets the number set in Votes each. The list sorts by votes. Reset votes clears the lot so the room can vote again.'], ['Prompt pack', 'What Claude writes for an idea so it can be built: an interview prompt for the person who owns the work, the skill or scheduled task itself, and the first message to send.'], ['Transcript source', 'Where the words come from: the Wispr Flow meeting, the laptop mic, dictation into the box, a JSON feed, or the demo.']] },
    { title: 'Keys', terms: [['1 to 8', 'Switch views.'], ['Space', 'Start or pause the block clock.'], ['N', 'Next block.'], ['P', 'Presentation mode on or off.'], ['Esc', 'Close a panel.']] }
  ];

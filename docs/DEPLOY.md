...
4. A new workshop is a blank slate: no systems, no process phases, no prepared second viewpoint ideas, and a run sheet with one process walk block per team you named. Add systems and phases in the board before the day (Systems → Add a system; Process walk → Add a phase), let participants add them from their phones (they can edit and delete their own), or let Claude add the ones the room names while Live capture runs. Votes per person is any number: set it when creating the workshop or later in Settings → This workshop. A workshop created before this change still carries the Watches of Switzerland template: Settings → This workshop → **Start blank** removes it and keeps the workshop's own opportunities, votes and transcript.
